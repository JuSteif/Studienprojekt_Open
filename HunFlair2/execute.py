...
def run_hunflair_global(text, tagger, splitter):
    """
    Teilt den Text in Sätze mittels SciSpacySentenceSplitter, berechnet für jeden Satz
    den globalen Offset im Originaltext, führt die NER-Vorhersage mit HunFlair2 durch und
    extrahiert globale Entitäten.
    Rückgabe:
...
def main():
    ...
    with open(csv_file, mode="w", newline="") as file:
        ...
        writer.writerow(["Run", "Runtime (s)", "F1 Score (bipartite contains rule)", "Precision", "Recall",
                         "CPU Energy (J)", "GPU Energy (J)", "CPU Power (avg, W)", "GPU Power (avg, W)", "GPU Power (max, W)",
